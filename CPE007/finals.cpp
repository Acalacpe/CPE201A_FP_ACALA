...
const string INVENTORY_FILE = "inventory.csv";  // Inventory stored in CSV file
const string USERS_FILE = "users.txt";          // User login credentials

// ================= FUNCTION DECLARATIONS =================
bool loginUser();
void registerUser();
void mainMenu(vector<Product> &inventory);
...
int main() {
    ...
    loadFromFile(inventory);  // Load existing inventory from file

    cout << "=== Welcome to ARIS (Automated Retail Inventory System) ===\n";
    ...
    do {
        ...
        if (choice == 1) {
            if (loginUser()) {
                mainMenu(inventory); // Enter inventory system after successful login
            }
        } 
        else if (choice == 2) {
            registerUser();
...
void addProduct(vector<Product> &inventory) {
    ...
    for (const auto &item : inventory) {
        if (item.name == p.name) {
            cout << "Product already exists. Try editing it instead.\n";
            return;
        }
    }
...
// Sell product and handle transaction
void sellProduct(vector<Product> &inventory) {
    if (inventory.empty()) {
    ...
    for (auto &p : inventory) {
        if (p.name == name) {
            ...
            saveToFile(inventory); // Save update instantly
...
void saveToFile(const vector<Product> &inventory) {
    ...
    // 🟩 Write CSV header here
    outFile << "Product,Price,Quantity\n";

    // 🟨 Then write all product data
    for (const auto &p : inventory) {
